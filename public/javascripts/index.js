...
const causes_explain = {
    ...
    'sea': '바다 또는 해양(영어: Sea)은 지구 표면에서 전체 또는 일부가 육지로 둘러싸인 거대한 소금물이다.해양 오염(海洋汚染, 영어: marine pollution) 또는 바다 오염( - 汚染)은 해양에 화학물질 또는 작은 조각(먼지) 들이 들어가 일으키는 해로운 효과를 말한다. 해양오염이 문제가 되는 것은, 잠재적인 독성 화학물질이 작은 입자들에 들러붙어 플랑크톤, 바다 밑의 생물들에 흡수된다는 것이다. 이들 중 대부분은 가라앉거나 먹이사슬을 따라 위쪽으로 집중된다. 또한, 많은 동물 사료에는 생선과 생선 기름이 포함되므로, 독성은 수 주 후에는 가축으로부터 얻어지는 가공식품이나 낙농 제품에서도 나타날 수 있다. 여기에는 고기, 달걀, 우유, 버터, 마가린 등도 포함된다.',
    ...
    'carbonDioxide': '이산화 탄소(二酸化炭素, 영어: carbon dioxide)는 탄소 원자 하나에 산소 원자 둘이 결합한 화합물이다.기체 상태일 때는 무색, 무취, 무미로 지구의 대기에도 존재하며, 화산 가스에도 포함되어 있다. 이산화 탄소는 온실기체로 작용하여, 지구복사를 통하여 우주공간으로 나가는 에너지 중 일부를 다시 지구로 되돌린다. 이러한 이산화 탄소의 성질은 지구의 에너지 평형을 깨트려서, 지구온난화의 원인으로 작용한다. 이산화 탄소는 화석연료와 같은 탄소를 포함한 물질을 완전 연소시킬 경우 생성되는데, 최근 화석연료의 사용이 크게 늘면서 이산화 탄소의 배출량도 증가하여 대기중의 이산화 탄소 농도가 증가하였고 이는 지구온난화를 더욱 심화시키는 요인으로 작용하고 있다.'
};
